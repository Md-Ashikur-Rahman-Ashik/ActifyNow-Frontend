import axios from "axios";
import { useEffect, useState } from "react";
import VolunteerCard from "../VolunteerCard/VolunteerCard";
import { ScrollRestoration } from "react-router-dom";
import { Helmet } from "react-helmet-async";
import { FiAlignJustify } from "react-icons/fi";
import { IoGrid } from "react-icons/io5";
import VolunteerTable from "./VolunteerTable";

const VolunteerNeeded = () => {
  const [volunteer, setVolunteer] = useState([]);
  const [searchItem, setSearchItem] = useState("");
  const [filteredCards, setFilteredCards] = useState(volunteer);
  const [loading, setLoading] = useState(true);
  const [changeGrid, setChangeGrid] = useState(true);
  const [changeTable, setChangeTable] = useState(false);
  // console.log(volunteer)
  // console.log(filteredCards)

  useEffect(() => {
    axios
      .get(
        "https://b9a11-server-side-md-ashikur-rahman-ashik.vercel.app/needVolunteer",
        { withCredentials: true }
      )
      .then((res) => {
        const data = res.data;
        setVolunteer(data);
        setLoading(false);
      });
  }, [volunteer]);

  const handleInputChange = (event) => {
    const searchTerm = event.target.value;
    setSearchItem(searchTerm);

    const filteredItems = volunteer.filter((filteredVolunteer) =>
      filteredVolunteer.postTitle
        .toLowerCase()
        .includes(searchTerm.toLowerCase())
    );

    setFilteredCards(filteredItems);
  };

  const handleGrid = () => {
    setChangeTable(false);
    setChangeGrid(true);
  };

  const handleTable = () => {
    setChangeGrid(false);
    setChangeTable(true);
  };

  if (loading) {
    return (
      <div className="flex justify-center items-center">
        <span className="loading loading-bars loading-xs"></span>
        <span className="loading loading-bars loading-sm"></span>
        <span className="loading loading-bars loading-md"></span>
        <span className="loading loading-bars loading-lg"></span>
      </div>
    );
  }

  return (
    <div className="container p-6 mx-auto min-h-[calc(100vh-349px)]">
      <Helmet>
        <title>All Volunteer Request Post | ActifyNow</title>
      </Helmet>
      <ScrollRestoration></ScrollRestoration>
      <fieldset className="w-full mb-5 flex justify-center space-y-1 text-gray-100">
        <label htmlFor="Search" className="hidden">
          Search
        </label>
        <div className="relative">
          <span className="absolute inset-y-0 left-0 flex items-center pl-2">
            <button
              type="button"
              title="search"
              className="p-1 focus:outline-none focus:ring"
            >
              <svg
                fill="currentColor"
                viewBox="0 0 512 512"
                className="w-4 h-4 text-gray-400"
              >
                <path d="M479.6,399.716l-81.084-81.084-62.368-25.767A175.014,175.014,0,0,0,368,192c0-97.047-78.953-176-176-176S16,94.953,16,192,94.953,368,192,368a175.034,175.034,0,0,0,101.619-32.377l25.7,62.2L400.4,478.911a56,56,0,1,0,79.2-79.195ZM48,192c0-79.4,64.6-144,144-144s144,64.6,144,144S271.4,336,192,336,48,271.4,48,192ZM456.971,456.284a24.028,24.028,0,0,1-33.942,0l-76.572-76.572-23.894-57.835L380.4,345.771l76.573,76.572A24.028,24.028,0,0,1,456.971,456.284Z"></path>
              </svg>
            </button>
          </span>
          <input
            value={searchItem}
            onChange={handleInputChange}
            type="search"
            name="Search"
            placeholder="Type to search"
            className="w-32 py-2 pl-10 text-sm rounded-md sm:w-auto focus:outline-none border-2 text-gray-400 border-gray-400 focus:border-violet-400"
          />
        </div>
      </fieldset>
      <div className="flex justify-end items-center gap-4 mb-5">
        <h2 className="font-bold">Change Layout</h2>
        <div
          className={`${changeTable && "text-blue-400"}`}
          onClick={handleTable}
        >
          <FiAlignJustify />
        </div>
        <span
          className={`${changeGrid && "text-blue-400"}`}
          onClick={handleGrid}
        >
          <IoGrid />
        </span>
      </div>
      {/* Grid Format */}
      {searchItem !== "" ? (
        <div
          className={`${
            changeGrid && "grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3"
          } ${!changeGrid && "hidden"}`}
        >
          {filteredCards.map((vol) => (
            <VolunteerCard key={vol._id} vol={vol}></VolunteerCard>
          ))}
        </div>
      ) : (
        <div
          className={`${
            changeGrid && "grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3"
          } ${!changeGrid && "hidden"}`}
        >
          {volunteer.map((vol) => (
            <VolunteerCard key={vol._id} vol={vol}></VolunteerCard>
          ))}
        </div>
      )}

      {/* Tabular format */}
      {searchItem !== "" ? (
        <div className={`${!changeTable && "hidden"}`}>
          <div className="overflow-x-auto">
            <table className="table">
              {/* head */}
              {volunteer?.length > 0 && (
                <thead>
                  <tr>
                    <th className="flex justify-center md:text-xl">Title</th>
                    <th className="md:text-xl">Category</th>
                    <th className="text-red-500 md:text-xl">Deadline</th>
                    <th></th>
                  </tr>
                </thead>
              )}
              <tbody>
                {filteredCards.map((vol) => (
                  <tr key={vol._id}>
                    <VolunteerTable vol={vol}></VolunteerTable>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      ) : (
        <div className={`${!changeTable && "hidden"}`}>
          <div className="overflow-x-hidden">
            <table className="table">
              {/* head */}
              {volunteer?.length > 0 && (
                <thead>
                  <tr>
                    <th className="flex justify-center md:text-xl">Title</th>
                    <th className="md:text-xl">Category</th>
                    <th className="text-red-500 md:text-xl">Deadline</th>
                    <th></th>
                  </tr>
                </thead>
              )}
              <tbody>
                {volunteer.map((vol) => (
                  <tr key={vol._id}>
                    <VolunteerTable vol={vol}></VolunteerTable>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      )}
    </div>
  );
};

export default VolunteerNeeded;
